...
x = 0
for FILE_NAME in glob.glob('uvb/*.he5'):
	x += 1
	if x > 1:
		break
	file = h5py.File(FILE_NAME, 'r')   # 'r' means that hdf5 file is open in read-only mode
	dataFields = file['HDFEOS']['GRIDS']['OMI UVB Product']['Data Fields']
	data = dataFields['UVindex'][:]
	print(data.shape)
	...
	#map_label=file['Geophysical Data'].attrs['Units'].decode()
	...
	my_cmap.set_under('w')
	m.pcolormesh(lon, lat, data, latlon=True, vmin=0, vmax=np.nanmax(data),cmap=my_cmap)
	cb = m.colorbar()
	cb.set_label('UV Index')
	plt.autoscale()
	#title the plot
	plt.title('{0}\n {1}'.format(FILE_NAME, 'UV Index'))
	fig = plt.gcf()
